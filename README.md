...
| **Объявления — API `banners`** | Читать, править и удалять объявления; создаются они вложенными в группу |
| **Контент** | Загружать изображения и видео из локального файла или по URL |
| **Статистика** | Получать данные по дням, итоги за период и срезы по возрасту, полу, гео, площадкам и устройствам |
...
- Объявления нельзя создать отдельным вызовом: в API нет такого метода, они заводятся вложенным массивом `banners` внутри группы объявлений. Отправки на модерацию в API тоже нет.
- Асинхронных отчётов в API нет — доступны обычная статистика по дням, итоги и срезы.
