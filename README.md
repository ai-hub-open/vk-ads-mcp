...
**Токен в адресе — для клиентов без заголовков.** Claude Desktop и подобные приложения позволяют указать только URL сервера. Для них включается отдельный эндпоинт: `MCP_ALLOW_URL_TOKEN=true`, после чего адрес выглядит так:

```
https://ваш-домен/t/<токен VK Ads>
```

Пользователю достаточно вставить этот адрес в настройки — больше ничего не требуется: ни установки программ, ни отдельного входа. Токен в адресе сам служит пропуском, поэтому `MCP_AUTH_TOKEN` для таких запросов не спрашивается (обычный `/mcp` продолжает его требовать). Учтите: адрес с токеном попадает в журналы обратного прокси-сервера — включайте режим осознанно и ограничьте частоту запросов.
